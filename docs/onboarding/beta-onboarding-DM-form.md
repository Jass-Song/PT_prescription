...
## 3.5. 환자 사전 스크리닝 — Red Flag 안전 확인 (필수 안내)

> **목적**: PT 선생님이 도수치료 시작 전 임상 안전망 (8 red flag 패턴)을 인지하고, 환자 인터뷰에서 스크리닝하도록 명시 안내. 앱 내 **"안전 확인 (선택)" 섹션** UI 와 1:1 매칭됩니다.
>
> **철학**: KMO — "Calm things down, and build things back up." 두려움 자극 없이 안전한 의뢰 흐름으로 안내합니다. 본 안내는 **추천 시스템이 PT 임상 판단을 대체하지 않는다**는 원칙을 분명히 합니다.

### 3.5-1. 8 패턴 빠른 안내 (앱 UI 라벨과 동일)

| # | 한국어 라벨 (앱 동일) | 임상 의미 (1줄) | 발견 시 처치 |
|---|---|---|---|
| 1 | **어지러움·구토·시야 이상** | VBI (척추동맥부전) 신호 — 복시·구음장애·기절 동반 시 특히 주의 | Cervical manipulation 보류 · 의료 의뢰 권고 |
| 2 | **안장 부위 감각 이상·대소변 조절 문제** | Cauda equina 신호 — 양측 하지 마비 동반 가능 | 도수치료 보류 · 즉시 의료 의뢰 권고 |
| 3 | **진행성 근력 약화·반사 변화** | Myelopathy 징후 (Hoffmann·Babinski 양성 등) | 도수치료 보류 · 의료 의뢰 권고 |
| 4 | **야간통 + 체중감소 + 발열** | 비기계적 통증 패턴 — 종양·감염 가능성 (50세 이상 신규 통증 포함) | 도수치료 보류 · 영상·혈액 검사 의뢰 권고 |
| 5 | **최근 외상 직후** | 골절·인대 파열 가능성 — 영상 진단 미실시 시 | 도수치료 보류 · 영상 진단 우선 권고 |
| 6 | **안정 시에도 강한 통증 + 진행성 악화** | 급성 reactive 상태 — 자극에 매우 민감 | Maitland Grade I 이하 또는 진정 우선 · 의료 의뢰 검토 |
| 7 | **급성 방사통 활동기** | 신경 자극 활동기 — manipulation 자극에 민감 | Manipulation 신중 · Mobilization·운동 우선 · 진행 시 의뢰 |
| 8 | **PFP (슬개대퇴 통증) 단독 도수 적용** | 단독 도수치료 효과 제한적 (CPG 근거) | Hip+Knee 운동 결합 권장 — 단독 도수 비권장 |

### 3.5-2. 사용 절차 (앱 UI 흐름)

1. 도수치료 시작 전 환자 인터뷰에서 위 8 패턴 확인
2. 해당 항목이 1개 이상이면 앱 화면의 **"⚠️ 안전 확인 (선택)"** 섹션을 펼쳐 체크
3. 1개 이상 체크 시 추천이 일시 멈추고 의료 의뢰 안내가 표시됨 (Phase 0-a 활성화 시)
4. 의뢰 결정은 PT 선생님의 임상 판단입니다 — 본 체크리스트는 보조 도구입니다

### 3.5-3. 의뢰 흐름 환자 안내 예시

> "지금 보이는 신호는 도수치료 전에 의사 선생님 진료를 한 번 받아보시는 게 좋겠어요. 큰 문제가 아닐 수도 있지만, 먼저 확인하고 진행하는 게 안전합니다."

- 환자의 불안을 키우는 단정 표현은 피합니다 ("이건 OO 입니다" → ✗)
- "확인 후 진행"이라는 절차적 안내로 자연스럽게 의뢰합니다
- 추천 카드의 의뢰 안내 모달 문구를 그대로 활용해도 됩니다

### 3.5-4. 면책 안내 (반드시 베타 PT 에게 전달)

- 본 추천 시스템은 임상 의사결정 **지원** 도구이며, PT 선생님의 **임상 판단을 대체하지 않습니다**.
- 환자 상태가 애매하면 보수적으로 의뢰를 권합니다.
- Red flag 체크리스트는 일반 권장이며 개별 환자 특성 (연령·동반질환·심리상태)은 임상가 판단으로 조정 필수입니다.

### 3.5-5. 참고 자료

- **빠른 참조 카드** (1페이지, 인쇄·휴대폰 즐겨찾기 권장): `docs/clinical/red-flag-quick-reference-2026-05-12.md`
- **상세 시나리오**: `docs/clinical-3axis-tier1-recommendation-research-2026-05-06.md` §5.5
- **출처**: APTA CPGs (Neck 2017 / LBP 2021 / Hip 2025 / Knee 2018 / PFP 2019 / Shoulder 2013·2020·2025 / Ankle 2021) + KAOMPT 안전 가이드 + Kerry & Taylor *JOSPT* 2009 (CAD)
