...
export default async function handler(req, res) {
  ...
  if (activeMT.length > 0) {
    ...
    // 환자 컨디션 기반 카테고리 점수로 정렬 후 그룹별 최대 선택
    ...
    // indication 적합도 순 평면 목록 — LLM은 순서 유지하며 재작성만
    ...
    allowedMTText = `Manual Therapy 기법 목록 (indication 적합도 순으로 정렬됨. 순서 변경·추가·제거 금지):\n\n` +
      allSelectedMT.join('\n\n');
  } else {
...
1. 자세·동작 설명은 초보 치료사도 바로 이해하는 쉬운 한국어 사용. 한자어·의학 약어 금지.
   ✗ 앙와위, 양와위, 측와위, 복와위, 반좌위, 복위, 전굴, 후굴, 측굴, 촉진, 신연, 거상, 표재층, 신전
   ✓ 등 대고 눕기(앙와위→), 옆으로 눕기(측와위→), 엎드려 눕기(복와위→), 등받이 세워 앉기, 앞으로 굽히기, 뒤로 젖히기, 옆으로 기울이기, 손으로 부드럽게 누르기, 팔 들어올리기, 피부 가까운 얕은 층(표재층→), 쭉 펴기(신전→)
...
▸ MT: 주어진 기법 목록의 순서를 유지하며 DB 정보를 쉬운 한국어로 재작성. 순서 변경·추가·제거 금지.
...
반환 형식 (JSON 외 출력 금지):
{
  ...
  "clinicalNote": "① [기법명]→[이 환자 상태에 적합한 임상 근거], [기법명]→[이유] ② 환자 임상 메시지 (가능성 표현, 전체 200자 이내)"
}
...
techniqueId는 [MT-XXX] 또는 [EX-XXX] ID를 그대로 복사.`;
  ...
  try {
    // MT 그룹당 최대 개수 강제 적용 (LLM 초과 선택 시 서버에서 잘라냄)
    // mt_soft: 6개, 나머지: 3개
