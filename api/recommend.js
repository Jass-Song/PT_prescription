...
// 전체 글로벌 스코어링: 모든 MT 기법을 condition score로 정렬 후 다양성 제약(그룹당 maxPerGroup) 적용
function selectTopTechniquesGlobally(activeMT, acuity, symptom, maxTotal = 6, maxPerGroup = 2) {
  const conditionScores = (CONDITION_CATEGORY_SCORES[acuity] || {})[symptom] || {};
  const scored = activeMT
    .map(t => ({
      ...t,
      score: conditionScores[t.category] ?? 0,
...
export default async function handler(req, res) {
  ...
  // topMT를 블록 밖에 선언: LLM 결과 확정 후 미선택 3개를 candidateTechniques로 반환하기 위함
  let topMT = [];
  let allowedMTText;
  if (activeMT.length > 0) {
    topMT = selectTopTechniquesGlobally(activeMT, acuity, symptom, 6, 2);
  ...
  const allowedEXText = topEX.length > 0
    ? `\n\n=== 운동 처방 후보 (상위 4개. 환자 상태 + 치료사 선호에 맞춰 2개 선택) ===\n` +
...
▸ MT: 후보 6개 중 이 환자(부위: ${region}, 시기: ${acuity}, 증상: ${symptom})에게 가장 적합한 3개를 임상적 우선순위 순서로 선택하여 DB 정보를 쉬운 한국어로 재작성.
▸ 운동: 치료사 선호 유형과 환자 상태를 고려하여 2개 선택
...
MT: 후보 중 3개 선택 (우선순위 순) / 운동처방: 2개 / targetMuscles: 최대 2개
techniqueId는 [MT-XXX] 또는 [EX-XXX] ID를 그대로 복사.`;
  ...
  try {
    ...
    // Claude가 선택하지 않은 나머지 후보 기법 — 프론트에서 "1회 교체" 버튼으로 표시
    const selectedIds = new Set((result.manualTherapy || []).map(item => item.techniqueId));
    result.candidateTechniques = topMT
      .filter(t => !selectedIds.has(t._promptId))
      .map(t => ({
        techniqueId:       t._promptId,
        name_ko:           t.name_ko,
        groupLabel:        MT_GROUP_LABEL[CATEGORY_TO_MT_GROUP[t.category]] || '기타',
        patientPosition:   t.patient_position  || '',
        therapistPosition: t.therapist_position || '',
        contactPoint:      t.contact_point      || '',
        direction:         t.direction          || '',
        steps:             Array.isArray(t.technique_steps) ? t.technique_steps.slice(0, 3) : [],
        categoryInfo:      (() => {
