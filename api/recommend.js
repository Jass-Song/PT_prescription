...
export default async function handler(req, res) {
  ...
  if (activeMT.length > 0) {
    ...
    allowedMTText = `사용 가능한 Manual Therapy 기법 목록:\n` +
      `⚠️ 규칙: 아래 각 === 섹션에서 반드시 딱 3개씩만 선택. 섹션 경계를 절대 넘지 말 것.\n\n` +
      preferredMT.map(mtId => {
        const label = MT_GROUP_LABEL[mtId] || mtId;
        const techs = techniquesByGroup[mtId] || [];
        const selectN = Math.min(3, techs.length);
        return `=== [${label}] (이 섹션에서 정확히 ${selectN}개 선택) ===\n${techs.map(t => formatTechniqueForPrompt(t, label)).join('\n\n')}`;
      }).join('\n\n');
  } else {
...
MT: 섹션당 N개(위 섹션 헤더에 명시된 수) / 운동처방: 3개 / targetMuscles: 최대 3개
