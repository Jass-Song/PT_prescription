...
export default async function handler(req, res) {
  ...
  if (activeMT.length > 0) {
    ...
    // 선택된 기법들만 indexedTechniques에 등록 (이미 위에서 전체 등록됐으므로 추가 처리 불필요)
    allowedMTText = `Manual Therapy 기법 목록 (아래 기법 전부를 임상 우선순위 순으로 정렬하고 쉬운 한국어로 재작성하세요. 추가·제거 금지):\n\n` +
      preferredMT.map(mtId => {
        const label = MT_GROUP_LABEL[mtId] || mtId;
        const techs = techniquesByGroup[mtId] || [];
        return `=== [${label}] (${techs.length}개 전부 포함) ===\n${techs.map(t => formatTechniqueForPrompt(t, label)).join('\n\n')}`;
      }).join('\n\n');
  } else {
...
▸ MT: 각 섹션에 주어진 기법 전부를 임상 우선순위 순으로 정렬하고 쉬운 한국어로 재작성 (추가·제거 금지)
...
반환 형식 (JSON 외 출력 금지):
{
  ...
  "clinicalNote": "① MT 추천 순서 근거: 1순위 [기법명]→[이유], 2순위 [기법명]→[이유] ② 환자 임상 메시지 (가능성 표현, 전체 200자 이내)"
}
